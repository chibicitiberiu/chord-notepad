...
The file has two tracks. Track 0 is a conductor track carrying tempo, meter and
key-signature metadata derived from the song's ``tempo_map``/``meter_map`` and
per-chord ``key``. Track 1 carries the voiced chords as note_on/note_off pairs on
channel 0. All timing comes from the beat domain of the render, converted to MIDI
ticks through the meter map, so the exported file matches playback exactly.
...
def _build_chord_track(
    ...
    """Build track 1: name, program change, and note_on/note_off per chord."""
    events: List[Tuple[int, int, mido.Message]] = []
    events.append((0, 0, mido.MetaMessage('track_name', name='Chords', time=0)))
...
def write_midi_file(
    ...
    Args:
        ...
        program: General MIDI program (instrument) number for the chord track.
    ...
    midi.tracks.append(_build_chord_track(rendered, b2t, program))
